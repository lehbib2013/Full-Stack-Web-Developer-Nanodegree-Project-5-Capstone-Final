...
journals_blueprint = Blueprint('journals_blueprint', __name__)

'''
              GET cashierJournal, return paginated results for CashierJournals\
              by specifying cashier,anne and mois arguments
              127.0.0.1:5000/journals?cashier_id=1&annee=2022&mois=2
              This endpoint should return a list of cashierjournals, by \
                specifying annee and mois arguments number,
'''


@journals_blueprint.route("/journals")
@requires_auth("get:everything")
def get_all_journals(jwt):
    mois = request.args.get('mois')
    annee = request.args.get('annee')
    cashier_id = request.args.get('cashier_id', None, type=int)
    print("test")
    if('mois' not in request.args or 'annee' not in request.args
            or 'cashier_id' not in request.args):
        try:
            current_journals = CashierJournal.query \
                .filter(CashierJournal.cashier_id == cashier_id) \
                .filter(CashierJournal.annee == annee) \
                .filter(CashierJournal.mois == mois) \
                .all()
            formatted_journals = [journ.format() for journ in
                                  current_journals]
            return jsonify({
                    'success': True,
                    'journals': formatted_journals,
                    'totalJournals': len(formatted_journals)
                })
...
@journals_blueprint.route("/cashiers", methods=['POST'])
...
@journals_blueprint.route("/cashiers", methods=['PATCH'])
